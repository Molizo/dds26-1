...
wait_for_join_target() {
    if [ -z "${JOIN_CLUSTER_HOST:-}" ]; then
        return
    fi

    attempt=0
    until rabbitmqctl -n "rabbit@${JOIN_CLUSTER_HOST}" status >/dev/null 2>&1; do
        attempt=$((attempt + 1))
        if [ "${attempt}" -ge 60 ]; then
            echo "Cluster join target rabbit@${JOIN_CLUSTER_HOST} did not become ready in time" >&2
            exit 1
        fi
        sleep 2
    done
